...
void ADC_init_(){
   ADCSRA |= (1 << ADEN)|(1 << ADSC);//(1 << ADATE) |  | )no prescalar|0x6;//with /32 prescalar
}/*
void ACD_MUXx1(){
...
unsigned char ADC_conversion(unsigned char admux){//expected only 8 bits of
	ADMUX=(1<<ADLAR)|0x0<<6|admux;//5V ref
	ADCSRA|=1<<ADSC;
	while(ADCSRA&(1<<ADSC));
	char data=ADCH;
